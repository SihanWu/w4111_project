...
def root():
  ...
  for result in cursor:
    ...
    customer_dic[result['uid']] = [result['username'],result['password'], result['address'],result['email'],result['phone']]
  cursor.close()
...
def login():
   ...
   #print customer_dic.keys()
   for uid in customer_dic.keys():
    
    customer_up[customer_dic[uid][0].strip()] = customer_dic[uid][1].strip()
    if customer_dic[uid][0].strip() == get_username:
      login_user = [uid, get_username]
      for seller in cursor:
        if seller['uid'] == uid:
          login_user.append(seller['sid'])



   #print customer_up
   print get_username
   if get_username in customer_up.keys() and customer_up[get_username] == get_password:
...
def insert():
  ...
  #.conn.execute('INSERT INTO standard_info (iid, brand, size, original_price, weight, name, link) VALUES (iid, brand, size, original_price, weight, name, link)');
  try:
    g.conn.execute(text(cmd), iid = new_iid, brand = brand, size = size, original_price = original_price, weight = float(weight), name = name, link = link);
  except:
  ...
  cmd = 'INSERT INTO products_post_has VALUES (:pid, :sid, :iid, :pname, :price, :numbe, :customized_description)';
  
  try:
    g.conn.execute(text(cmd), pid = new_pid, sid = sid, iid = new_iid, pname = name, price = price, numbe = number, customized_description = customized_description);
  except:
    render_template('error.html'), 404
  print "sucessful"
